...
struct cgrp_control *cgrps[] = {
	&(struct cgrp_control){
		...
		.settings = (struct cgrp_setting *[]){
			...
			&add_to_tasks,
			NULL}},
	&(struct cgrp_control){.control = "cpu", .settings = (struct cgrp_setting *[]){&(struct cgrp_setting){.name = "cpu.shares", .value = SHARES}, &add_to_tasks, NULL}}, &(struct cgrp_control){
																																											 .control = "pids",
																																											 .settings = (struct cgrp_setting *[]){&(struct cgrp_setting){.name = "pids.max", .value = PIDS}, &add_to_tasks, NULL},
																																										 },
	&(struct cgrp_control){.control = "blkio", .settings = (struct cgrp_setting *[]){&(struct cgrp_setting){.name = "blkio.weight", .value = PIDS}, &add_to_tasks, NULL}}, NULL};
...
int resources(struct child_config *config)
{
	...
	for (struct cgrp_control **cgrp = cgrps; *cgrp; cgrp++)
	{
		...
		{
			fprintf(stderr, "mkdir %s falied: %m\n", dir);
			return -1;
		}
		for (struct cgrp_setting **setting = (*cgrp)->settings; *setting; setting++)
		{
			...
			{
				fprintf(stderr, "writting to %s failed: %m\n", path);
				close(fd);
				return -1;
			}
			close(fd);
...
int handle_child_uid_map(pid_t child_pid, int fd)
{
	...
	{
		char path[PATH_MAX] = {0};
		for (char **file = (char *[]){"uid_map", "gip_map", 0}; *file; file++)
		{
...
finish_options:
	...
	//TODO: Check for new Kernel version capabilities and system calls. Remove for testing purposes only. Removing will cause cgroups memory issues.
